...
# Read in file of curated chemicals (generated by Tony) that now has DTXSIDs we can use to query the dashboard for other metadata
...
write.table(curatedDTXSIDs, file = paste(myPath, "data", "houseProdSSA_curatedChems_DTXSIDs.txt", sep = "/"), quote = FALSE, row.names = FALSE, col.names = FALSE)
...
# Will carry these through all my other data objects so all data uses DTXSID identifiers
...
# Save tps as my master toxprints file
...
# Save plot
mycol <- ggthemes::excel_new_pal(theme = "Office Theme")[length(levels(pdb$Dilution))]
...
pdb$Dilution <- factor(pdb$Dilution)
pdb$Category <- factor(pdb$Category)
ggplot(data = pdb, aes(x = X1, y = X2, shape = Category, color = Dilution)) +
  geom_point() 
...
prodType <- paste(prodGroups, unlist(lapply(binary, function(x) dim(x)[2])))
allComb_BinAndConc_concat <- list("binary" = binary, "conc" = conc, "prodType" = prodType)
...
# Need to pull both dashboard files (matched and curated)
tps1 <- read_xlsx(paste(myPath, "data/houseProdSSA_chems_CCDquery_byName_2024-05-03.xlsx", sep = "/"), sheet = 2)
tps2 <- read_xlsx(paste(myPath, "data/houseProdSSA_curatedChems_CCDquery_byDTXSID.xlsx", sep = "/"), sheet = 2)
tps <- rbind(tps1, tps2)

# Reduce to identifiers and ToxPrints
colnames(tps)[1:50]
# Keep up to QSAR SMILES and then just tps (starts at 47th column)
tps <- tps[,c(1:7,47:dim(tps)[2])]

# Check for rows that didn't match. Can't get predicted uses without a proper ID.
as.data.frame(table(tps$FOUND_BY))
# There are a lot of different values this column can have, but if there is no match,
# the toxprints will be NA
sum(is.na(tps$`atom:element_main_group`))  # 91 chemicals
todrop <- is.na(tps$`atom:element_main_group`)
curatedTPs <- tps[!todrop,]  # 457 chemicals
# There are cases where one chemical returned multiple rows. Usually has Approved Name - Warning
# mapped to two or more chemicals. These are accompanied by 1 or 2 other rows that we can drop.
todrop <- c("Integrated Source Name - WARNING: Synonym mapped to two or more chemicals",
            "Synonym - WARNING: Synonym mapped to two or more chemicals")
curatedTPs <- curatedTPs[!curatedTPs$FOUND_BY %in% todrop,]
# Drop any remaining duplicated DTXSIDs
curatedTPs <- curatedTPs[!duplicated(curatedTPs$DTXSID),]  # final set of 449 chemicals
rm(tps1, tps2, tps, todrop)
...
# Get predictions for the rest. First line is for only predicting for chemicals not in FUse. 
# Latter is for predicting for all, which is what I used for the manuscript.
...
# Use the object I saved with all raw and curated chemical identifiers
load(file = paste(myPath, "data", "rawAndCuratedChemIdentifiers_xcxt.RData", sep = "/"))  # loads chemIdentifiers object
ind <- match(use.pred.rep$DTXSID, curatedTPs$DTXSID)
ind <- match(use.pred.rep$DTXSID, chemIdentifiers$DTXSID)
use.pred.rep <- as.data.frame(use.pred.rep)
row.names(use.pred.rep) <- curatedTPs$PREFERRED_NAME[ind]
