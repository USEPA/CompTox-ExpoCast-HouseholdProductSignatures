...
For clustering and upset plots, remove the blanks, combine dilutions, combine
duplicates, combine repeats.  But first, create matrices without combining samples
to assess within-product variability.
...
# Also include a vector to indicate the product names so we know which are repeats
ind <- allSamps$Product == "Clothing"
clothing_mats <- list("binary" = chemXsamp_binary_red[,ind], "conc" = chemXsamp_conc_red[,ind], 
                      "group" = chemXsamp_group_red[,ind], "prod_names" = allSamps$`Customer ID`[ind])
ind <- allSamps$Product == "Fabric"
fabric_mats <- list("binary" = chemXsamp_binary_red[,ind], "conc" = chemXsamp_conc_red[,ind], 
                      "group" = chemXsamp_group_red[,ind], "prod_names" = allSamps$`Customer ID`[ind])
ind <- allSamps$Product == "Kitchen"
kitchen_mats <- list("binary" = chemXsamp_binary_red[,ind], "conc" = chemXsamp_conc_red[,ind], 
                      "group" = chemXsamp_group_red[,ind], "prod_names" = allSamps$`Customer ID`[ind])
ind <- allSamps$Product == "Personal Care Products"
personalcare_mats <- list("binary" = chemXsamp_binary_red[,ind], "conc" = chemXsamp_conc_red[,ind], 
                      "group" = chemXsamp_group_red[,ind], "prod_names" = allSamps$`Customer ID`[ind])
ind <- allSamps$Product == "Shampoo"
shampoo_mats <- list("binary" = chemXsamp_binary_red[,ind], "conc" = chemXsamp_conc_red[,ind], 
                      "group" = chemXsamp_group_red[,ind], "prod_names" = allSamps$`Customer ID`[ind])

prodList <- list(clothing_mats, fabric_mats, personalcare_mats, shampoo_mats, kitchen_mats)
names(prodList) <- c("Cotton Clothing", "Fabric Upholstery", "Baby Soap", "Shampoo", "Silicone Kitchen Tools")
...
### Analysis Using Functional Uses
```{r funcUse}
...
# Chemical name by functional use, so I need to pull my occurrence matrices for each product category
# to get a product by use matrix
...
clustProdConcMat <- function(conc_mat, prod_name, funUses, myPath){
  ...
  # Rank heatmap for uses to use as annotation
  
  
  # Build annotations for these
  palettes <- ggthemes_data[["tableau"]][["color-palettes"]][["regular"]]
  cols <- palettes$`Tableau 20`$value[1:length(unique(uses))]
  names(cols) <- unique(uses)
  type_col <- palettes$`Color Blind`$value[2:(length(unique(prodOnt$Group3[ind]))+1)]
  names(type_col) <- unique(prodOnt$Group3[ind])

  column_ha <- HeatmapAnnotation(Brand = prodOnt$Brand[ind], Type = prodOnt$Group3[ind],  
                                 col = list(Brand = brand_col, Type = type_col))
  ...
  temp <- temp/1000
  colnames(temp) <- clothing_mats$prod_names
  ...
  # Make plot
  pdf(file = paste(myPath, "plots", "prodSpecificHeatmap_conc_Clothing.pdf", sep = "/"), width = 10, height = 13)
  Heatmap(temp, top_annotation = column_ha, right_annotation = row_ha, left_annotation = left_ha,
          row_title = "Chemicals", column_title = "Clothing Samples", cluster_rows = FALSE,
          cluster_columns = TRUE, show_row_names = FALSE, column_names_rot = 90, column_names_gp = gpar(fontsie = 2), 
...
ht <- Heatmap(temp, name = "Concentration (ug/g)", top_annotation = column_ha, right_annotation = row_ha, 
              left_annotation = left_ha, row_title = "Chemicals", column_title = "Cotton Clothing Samples", 
...
pdf(file = paste(myPath, "plots", "prodSpecificHeatmap_conc_Clothing_newUseAnno_v2.pdf", sep = "/"), width = 11, height = 13)
...
# Put in ug/g rather than ng/g
temp <- temp/1000
colnames(temp) <- personalcare_mats$prod_names
column_ha <- HeatmapAnnotation(Total_Conc_Sample = unlist(apply(temp, 2, function(x) sum(x, na.rm = TRUE))), 
                               Chems_in_Sample = anno_barplot(unlist(apply(temp, 2, function(x) sum(!is.na(x))))))
row_ha <- rowAnnotation(Total_Conc_Chem = unlist(apply(temp, 1, function(x) sum(x, na.rm = TRUE))), annotation_name_rot = 90,
                            Samples_with_Chem = anno_barplot(unlist(apply(temp, 1, function(x) sum(!is.na(x))))))
...
# Make plot
pdf(file = paste(myPath, "plots", "prodSpecificHeatmap_conc_PersonalCare.pdf", sep = "/"), width = 10, height = 13)
Heatmap(temp, top_annotation = column_ha, right_annotation = row_ha, left_annotation = left_ha,
        row_title = "Chemicals", column_title = "Personal Care Product Samples", cluster_rows = FALSE,
        cluster_columns = TRUE, show_row_names = FALSE, column_names_rot = 90, column_names_gp = gpar(fontsie = 2), 
        column_names_max_height = unit(4, "cm"))
dev.off()
...
# Put in ug/g rather than ng/g
temp <- temp/1000
colnames(temp) <- fabric_mats$prod_names
column_ha <- HeatmapAnnotation(Total_Conc_Sample = unlist(apply(temp, 2, function(x) sum(x, na.rm = TRUE))), 
                               Chems_in_Sample = anno_barplot(unlist(apply(temp, 2, function(x) sum(!is.na(x))))))
row_ha <- rowAnnotation(Total_Conc_Chem = unlist(apply(temp, 1, function(x) sum(x, na.rm = TRUE))), annotation_name_rot = 90,
                            Samples_with_Chem = anno_barplot(unlist(apply(temp, 1, function(x) sum(!is.na(x))))))
...
# Make plot
pdf(file = paste(myPath, "plots", "prodSpecificHeatmap_conc_Fabric.pdf", sep = "/"), width = 10, height = 13)
Heatmap(temp, top_annotation = column_ha, right_annotation = row_ha, left_annotation = left_ha,
        row_title = "Chemicals", column_title = "Fabric Samples", cluster_rows = FALSE,
        cluster_columns = TRUE, show_row_names = FALSE, column_names_rot = 90, column_names_gp = gpar(fontsie = 2), 
        column_names_max_height = unit(4, "cm"))
dev.off()
...
# Put in ug/g rather than ng/g
temp <- temp/1000
colnames(temp) <- kitchen_mats$prod_names
column_ha <- HeatmapAnnotation(Total_Conc_Sample = unlist(apply(temp, 2, function(x) sum(x, na.rm = TRUE))), 
                               Chems_in_Sample = anno_barplot(unlist(apply(temp, 2, function(x) sum(!is.na(x))))))
row_ha <- rowAnnotation(Total_Conc_Chem = unlist(apply(temp, 1, function(x) sum(x, na.rm = TRUE))), annotation_name_rot = 90,
                            Samples_with_Chem = anno_barplot(unlist(apply(temp, 1, function(x) sum(!is.na(x))))))
...
# Make plot
pdf(file = paste(myPath, "plots", "prodSpecificHeatmap_conc_Kitchen.pdf", sep = "/"), width = 10, height = 13)
Heatmap(temp, top_annotation = column_ha, right_annotation = row_ha, left_annotation = left_ha,
        row_title = "Chemicals", column_title = "Kitchen Samples", cluster_rows = FALSE,
        cluster_columns = TRUE, show_row_names = FALSE, column_names_rot = 90, column_names_gp = gpar(fontsie = 2), 
        column_names_max_height = unit(4, "cm"))
dev.off()
...
# Put in ug/g rather than ng/g
temp <- temp/1000
colnames(temp) <- shampoo_mats$prod_names
column_ha <- HeatmapAnnotation(Total_Conc_Sample = unlist(apply(temp, 2, function(x) sum(x, na.rm = TRUE))), 
                               Chems_in_Sample = anno_barplot(unlist(apply(temp, 2, function(x) sum(!is.na(x))))))
row_ha <- rowAnnotation(Total_Conc_Chem = unlist(apply(temp, 1, function(x) sum(x, na.rm = TRUE))), annotation_name_rot = 90,
                            Samples_with_Chem = anno_barplot(unlist(apply(temp, 1, function(x) sum(!is.na(x))))))
...
# Make plot
pdf(file = paste(myPath, "plots", "prodSpecificHeatmap_conc_Shampoo.pdf", sep = "/"), width = 10, height = 13)
Heatmap(temp, top_annotation = column_ha, right_annotation = row_ha, left_annotation = left_ha,
        row_title = "Chemicals", column_title = "Shampoo Samples", cluster_rows = FALSE,
        cluster_columns = TRUE, show_row_names = FALSE, column_names_rot = 90, column_names_gp = gpar(fontsie = 2), 
        column_names_max_height = unit(4, "cm"))
dev.off()
...
#################################################################################################
# Calculate a sample X sample distance matrix for binary data
...
for (i in 1:length(prodList)){
  if (names(prodList)[i] == "Fabric Upholstery"){
    ...
    #column_ha <- HeatmapAnnotation(Chems_in_Sample = anno_barplot(unlist(apply(prodList[[i]]$binary, 2, function(x) sum(x!=0)))))
    ...
    #type_col <- palettes$`Color Blind`$value[2:(length(unique(temp$Group3))+1)]
    #names(type_col) <- unique(temp$Group3)
    
    #brand_col <- brewer.pal(length(unique(temp$Brand)), "Pastel2")
    #names(brand_col) <- unique(temp$Brand)
...
for (i in 1:length(prodList)){
  ...
  #signatures[[i]] <- log10(signatures[[i]])
  # Get mean and sd
  #signatures[[i]] <- cbind(signatures[[i]], t(apply(signatures[[i]], 1, function(x) c(mean(x, na.rm = TRUE), sd(x, na.rm = TRUE)))))
  #ind <- dim(signatures[[i]])[2]
  #colnames(signatures[[i]])[(ind-1):ind] <- c("Mean_Conc", "SD_Conc")
}
...
for (i in 1:length(prodList)){
  ...
  #ind <- dim(signatures[[i]])
  #df <- rbind(df, data.frame("Product_Type" = rep(names(prodList)[i], ind[1]), "Chemical" = row.names(signatures[[i]]), 
   #           signatures[[i]][,(ind[2]-1):ind[2]], row.names = NULL))
}
...
#g <- ggplot(df, aes(x = reorder_within(Chemical, -Mean_Conc, Product_Type), y = Mean_Conc, fill = Product_Type)) +
#  geom_bar(stat = "identity", colour = "black") +
#  geom_errorbar(aes(ymin = Mean_Conc - SD_Conc, ymax = Mean_Conc + SD_Conc), size = 0.5, width = 0.25) +
#  scale_x_reordered() +
#  facet_grid(~Product_Type, scales = "free_x", space = "free") +
#  theme_bw() +
#  ylab("Mean log10(Conc) (ng/g)") +
#  ggtitle("Chemicals Present in at least 80% of Product Samples") +
#  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) +
#  theme(axis.title.x = element_blank())
#pdf(file = paste(myPath, "plots", "mostPrevChemsPerProduct_MeanConc.pdf", sep = "/"), width = 12, height = 7)
#print(g)
#dev.off()
  

################################
# Do a simple classification of Katherine Phillips' pilot study samples using these signatures
...
length(intersect(row.names(sigList$`Cotton Clothing`), row.names(sigList$`Fabric Upholstery`)))
length(intersect(row.names(sigList$`Baby Soap`), row.names(sigList$Shampoo)))
sets <- lapply(sigList, row.names)
intersect(sets$`Cotton Clothing`, sets$`Baby Soap`)
intersect(sets$`Cotton Clothing`, sets$Shampoo)
...
lapply(uniqSigList, dim)  # 9 for clothing, 5 for fabric, 4 for soap, 6 for shampoo, 7 for kitchen


# From Phillips et al 2018, pilot study with 20 product types (5 samples each)
...
#sigCAS <- unique(unlist(lapply(sigList, function(x) x[,4])))
...
###############################################################
# Do for multiple different levels than just the above 5 categories
load(paste(myPath, "data", "allProdTypesMat_combDilDupRep.RData", sep = "/"))
...
# Verify Upset Plot
length(setdiff(upset_list$Clothing, do.call("c", upset_list[2:5])))  # Yes
length(setdiff(upset_list$Fabric, do.call("c", upset_list[1:4])))  # yes
#length(do.call("intersect", upset_list[1:5]))
...
#chemSums <- chemSums/length(sampNames)
#sum(chemSums < 0.05)
...
# Try making my own clusters for chemicals
n_clusters <- 20
...
```{r sparsemodel}
# Couldn't install glmnet.  Run on Katherine Phillips' workstation
#load(paste(myPath, "data", "prodList_all5cats_BinaryConcGroupNames.RData", sep = "/"))
load(paste(myPath, "data", "allProdTypesMat_combDilDupRep.RData", sep = "/"))
...
## Don't need glmnet, can do using the caret package
# Need the final chemical X sample occurrence matrix, which was saved in the last code block
# after combining repeats and duplicates
...
# Use CAS since I already wrote code that way for Charlie's dataset
...
########################################
## Now predict the product type of an external set of samples
# From Lowe et al 2021 in ES&T SSA of Recycled Products
occurMat <- read.csv(file = paste(myPath, "data", "Lowe2021_SSA_Recycled_Prods_OccurMat.csv", sep = "/"))
sampList <- as.data.frame(read_xlsx(path = paste(myPath, "data", "Lowe2021_SSA_Recycled_Prods_SupplData.xlsx", sep = "/"), sheet = 2))
# Need to extract the column names, which were altered when reading in the file
row.names(occurMat) <- occurMat$X
occurMat$X <- NULL
extChems <- colnames(occurMat)
extChems <- gsub("X", "", extChems)
extChems <- gsub("\\.", "\\-", extChems)
# Load my dashboard file to match these CAS with my chemical names
myChems <- read_xlsx(paste(myPath, "data/houseProdNTA_chems_CCDquery_byName_full.xlsx", sep = "/"), sheet = 2)
myChems <- myChems[,1:5]
# Drop chemicals that didn't have a match
todrop <- c("Searched by Synonym: Found 0 results", "Integrated Source Name - WARNING: Synonym mapped to two or more chemicals",
...
# Build DF for external test set
extDF <- as.data.frame(matrix(data = NA, nrow = dim(occurMat)[1], ncol = dim(featMat)[2], 
                              dimnames = list(row.names(occurMat), colnames(featMat))))
for (i in 1:dim(extDF)[2]){
  ind1 <- which(myChems$INPUT == colnames(extDF)[i])
  if (length(ind1) == 0){
    extDF[,i] <- 0
  } else {
    ind2 <- which(extChems == myChems$CASRN[ind1])
    if (length(ind2) == 0){
      ...
      extDF[,i] <- occurMat[,ind2]
    }
  }  
}

# Extract the samples we want to test 
indF <- which(sampList$Classification == "Fabric Products")
indRug <- grep("rug", sampList$`General Product Description or Product Type`)
indRubber <- grep("rubber", sampList$`General Product Description or Product Type`)
indMat <- grep("mat", sampList$`General Product Description or Product Type`)
