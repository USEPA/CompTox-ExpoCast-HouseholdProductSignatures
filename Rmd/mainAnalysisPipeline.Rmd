...
for (i in 1:length(prodGroups)){
  ...
  # High Level
  useFlags <- apply(preds[catChems,-1], 1, function(x) ifelse(any(x > 0.9999), "Reported", 
  ...
  prodCatUses[[i]] <- cbind(colnames(temp), gsub(" ", "_", refTable$`Anon Sample`[indSamp]), 
...
colnames(allCatUses) <- c("Category", "Unique Chemicals", "Reported Use", "Predicted Use", "No Use")
...
df$Annotation <- factor(df$Annotation, levels = c("Reported Use", "Predicted Use", "No Use"), 
                        ordered = TRUE)
...
for (i in 1:length(prodGroups)){
  ...
  if (prodGroups[i] == "Fabric Upholstery"){
    ht <- Heatmap(temp, col = col_fun, 
                  heatmap_legend_param = list(title = "Unique Chemicals", at = mySeq, labels = as.character(mySeq)), 
                  na_col = "white", row_title = "Samples", 
                  column_title = paste("Reported Functional Use ", prodGroups[i], sep = ""),
                  cluster_rows = TRUE, cluster_columns = FALSE, show_row_names = TRUE, show_column_names = TRUE,
                  column_names_rot = 90, rect_gp = gpar(col = "white", lwd = 2, cex = 1.3))
  } else {
    ht <- Heatmap(temp, col = col_fun, 
                  heatmap_legend_param = list(title = "Unique Chemicals", at = mySeq, labels = as.character(mySeq)), 
                  na_col = "white", left_annotation = column_ha, row_title = "Samples", 
                  column_title = paste("Reported Functional Uses in ", prodGroups[i], sep = ""),
                  cluster_rows = TRUE, cluster_columns = FALSE, show_row_names = TRUE, show_column_names = TRUE,
                  column_names_rot = 90, rect_gp = gpar(col = "white", lwd = 2, cex = 1.3))
  }
...
prodOnt <- as.data.frame(read_xlsx(path = paste(myPath, "data", "masterSampleTable_withLevelsAndBrands.xlsx", 
                                                sep = "/")))
# Remove blanks
...
for (i in 1:length(prodList)){
  if (names(prodList)[i] == "Fabric Upholstery"){
    ...
    if (plot){
      pdf(file = paste(myPath, "plots/singleProdCatSimilarityHeatmaps", paste("withinProdSimilarityHeatmap_", names(prodList)[i], 
...
for (i in 1:length(prodGroups)){
  ...
  for (j in 1:length(indDups)){
    indreg <- colnames(distances[[i]]) == gsub("_dup", "", colnames(distances[[i]])[indDups[j]])
  ...
  sdRep <- sd(tempRep)
  # Build up data frame
  simTable <- rbind(simTable, c(avgTot, sdTot, avgRed, sdRed, length(indDups), avgDup, sdDup, length(indReps), avgRep, sdRep))
}
...
for (i in 1:length(prodList)){
  ...
  ht <- Heatmap(temp, name = "Concentration (ug/g)", top_annotation = column_ha, right_annotation = row_ha, 
                left_annotation = left_ha, row_title = "Chemicals", 
  ...
  pdf(file = paste(myPath, "plots/singleProdCatConcHeatmaps", paste("prodSpecificHeatmap_conc_", prodGroups[i], "_newUseAnno_v2.pdf", sep = ""),
...
g <- ggplot(df, aes(x = reorder_within(Chemical, -Mean, Product_Type), y = Conc, fill = Product_Type)) +
  ...
  facet_grid(~Product_Type, scales = "free_x", space = "free", 
             labeller = labeller(Product_Type = label_wrap_gen(5))) +
  theme_bw() +
...
## Compare with ChemExpo
To add more context to our results and signatures, bring in reported chemical
ingredient data using the EPA's ChemExpo Database, which has product composition
data, product use categories, and chemical weight fractions.

```{r ChemExpo}
# Read in file containing data for UPCs in our study that were also in ChemExpo
matchedData <- as.data.frame(read_xlsx(path = paste(myPath, "data",
                                                    "cpdat_upcs_matching_nta_zstanfield.xlsx", sep = "/")))
# How many products and chemicals per product?
as.data.frame(table(matchedData$upc))
# What categories do they belong to?
as.data.frame(table(matchedData$gen_cat))
as.data.frame(table(matchedData$prod_fam))
as.data.frame(table(matchedData$prod_type))

# Decide grouping and get signatures
# My baby soap samples are where the prod_fam column == body hygiene and child specific
# My shampoo samples are where the prod_fam column == hair styling and care
numprodtypes <- list(c("child specific", "body hygiene"), "hair styling and care")
prodCounts <- vector(mode = "list", length = length(numprodtypes))
for (i in 1:length(numprodtypes)){
  temp <- matchedData[matchedData$prod_fam %in% numprodtypes[[i]],]
  prodCounts[[i]] <- as.data.frame(table(temp$true_cas))
}

# Use my simple signatures: run the start of the classify external data section
# to get sigList
# Just look to see if any of the signature chemicals were reported in one of these products
names(sigList) <- prodGroups
# Baby Soap
BSoverlap <- row.names(sigList$`Baby Soap`)[sigList$`Baby Soap`$CAS %in% prodCounts[[1]]$Var1]
# Shampoo
SHoverlap <- row.names(sigList$Shampoo)[sigList$Shampoo$CAS %in% prodCounts[[2]]$Var1]


# For the categories we didn't have matches for, use the bulk download composition data from ChemExpo
# This is a very large file:  3,387,287 rows and 26 columns
chemExpo <- read.csv(file = "/ccte/home1/zstanfie/ChemExpo_bulk_composition_chemicals.csv",
                     header = TRUE)
prodchemDist <- as.data.frame(table(chemExpo$Product.Name))
summary(prodchemDist$Freq)
pucKindDist <- as.data.frame(table(chemExpo$PUC.Kind))
pucGenCatDist <- as.data.frame(table(chemExpo$PUC.General.Category))
pucProdFamDist <- as.data.frame(table(chemExpo$PUC.Product.Family))
pucProdTypeDist <- as.data.frame(table(chemExpo$PUC.Product.Type))
sum(pucProdFamDist$Freq > 5000)

# For each of our signatures, look to see how many of the chemicals occur in 
# the most well-represented product families in ChemExpo
pucProdFamDist <- pucProdFamDist[-1,]
prevFams <- as.character(pucProdFamDist$Var1[pucProdFamDist$Freq >= 5000])
appearInPrevFams <- as.data.frame(matrix(data = NA, nrow = length(prevFams), ncol = length(sigList), 
                                         dimnames  = list(prevFams, names(sigList))))
prevFamChems <- c()
for (i in 1:length(prevFams)){
  indFam <- chemExpo$PUC.Product.Family == prevFams[i]
  famChems <- unique(chemExpo$Curated.CAS[indFam])
  prevFamChems <- c(prevFamChems, length(famChems))
  appearInPrevFams[i,] <- lapply(sigList, function(x) length(intersect(x[,4], famChems)))
}

# Just see how many signature chemicals are reported in any product in ChemExpo
uniqueCEchems <- unique(chemExpo$Curated.CAS)
appearInChemExpo <- lapply(sigList, function(x) length(intersect(x[,4], uniqueCEchems)))


### Pull the ORAL RAT LD50 TEST estimates for our signature chemicals
# Bring in the dashboard result for my chemicals and remove those without a dashboard entry
myChems <- as.data.frame(read_xlsx(paste(myPath, "data/houseProdNTA_chems_CCDquery_byName_full.xlsx", 
                                         sep = "/"), sheet = 2))
indTox <- which(colnames(myChems) == "ORAL_RAT_LD50_MOL/KG_TEST_PRED")
...
for (i in 1:length(sigList)){
  indDash <- match(row.names(sigList[[i]]), myChems$INPUT)
  sigList[[i]] <- cbind(sigList[[i]], "InChemExpo" = sigList[[i]][,4] %in% uniqueCEchems,
                        "ORAL_RAT_LD50_MOL/KG" = myChems$`ORAL_RAT_LD50_MOL/KG_TEST_PRED`[indDash])
}

# Make a plot
df <- c()
for (i in 1:length(sigList)){
  df <- rbind(df, cbind(rep(names(sigList)[[i]], dim(sigList[[i]])[1]), sigList[[i]]$CAS, row.names(sigList[[i]]),
                            sigList[[i]]$InChemExpo, sigList[[i]]$`ORAL_RAT_LD50_MOL/KG`, sigList[[i]]$Mean))
  
}
colnames(df) <- c("ProductType", "CAS", "ChemicalName", "ReportedInChemExpo", "OralRatLD50molperkg", "Mean")
df <- as.data.frame(df)

df$ProductType <- factor(df$ProductType, levels = prodGroups[c(1,4,5,2,3)], ordered = TRUE)
df$OralRatLD50molperkg <- as.numeric(df$OralRatLD50molperkg)
df$Mean <- as.numeric(df$Mean)
g <- ggplot(df, aes(x = reorder_within(ChemicalName, -Mean, ProductType), 
                           y = OralRatLD50molperkg, fill = ReportedInChemExpo)) +
  geom_bar(stat = "identity", width = 0.8) +
  scale_x_reordered() +
  scale_fill_manual(values = c("darkblue", "darkgreen")) +
  facet_grid(~ProductType, scales = "free_x", space = "free",
             labeller = labeller(ProductType = label_wrap_gen(5))) +
  theme_bw() +
  ...
  theme(text =element_text(size = 13)) +
  theme(legend.text = element_text(size = 10))
pdf(file = paste(myPath, "plots", "prodCatSignatures_withChemExpoAndLD50.pdf", sep = "/"), width = 12, height = 7)
  print(g)
dev.off()
